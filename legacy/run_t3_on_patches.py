...
# -*- coding: utf-8 -*-
"""
Run T3/ARL-RG auf echten Astro-Patches (Planck/WMAP/HI4PI).
Liest die von make_real_patches_standalone.py erzeugten Patches (meta.json),
aggregiert über Patches, passt Trend (log|inv|auto), berechnet Plateau
und schreibt CSV/PNG/JSON in data/processed/astro/suite/<dataset>/.

Neu:
- Surrogate-Nulltest (--null N, --null-seed): Phasen-Surrogates pro Patch,
  Outputs *_null_metrics{_per_patch}.csv, *_null_report.png, *_null_summary.json
- Bugfix: ds/dataset_name in run_dataset() vereinheitlicht
- Plateau-Metrik robust (MAD-ähnlich via Spannweite / |Mittelwert|)
- AIC-gestützte Trendwahl bei --trend auto
"""
...
def _plot_and_write(ds: str, outdir: Path, agg: pd.DataFrame, theta: float, chosen: str,
    ...
    # Panel 1: Skalenfunktion mit Fehlerbalken + Trendabzug
...
def run_dataset(
    ...
    ds = dataset_name  # konsistent verwenden
    ...
    # Versuch, ein sinnvolles Masken-/Subset-Label zu extrahieren
    mask_label = (
        meta.get("mask_label")
        or meta.get("mask")
    ...
    # Pfade der Patches (absolut robust machen)
    ...
    if null_n and null_n > 0:
        ...
        # Trend für Null fixieren. Bei auto wird die Wahl durch die Daten festgelegt.
        ...
        # Referenz s und n_cg einmalig bestimmen.
        ...
        # Zielarrays. Pro Wiederholung bereits über Patches aggregiert.
        ...
        # Null Referenzen und Effekt Verteilungen.
        ...
        # Repräsentative Null Kurve. Median über reps.
        ...
        # std über reps wie pandas std, ddof=1.
        # Bei null_n == 1 würde ddof=1 zu NaN führen, da Freiheitsgrade fehlen.
        # Für null_n == 1 verwenden wir ddof=0, dann ist std = 0 und das Fehlerband ist definiert.
...
def main() -> None:
    ...
    ap.add_argument("--dataset", type=str, default=None,
                    help="z.B. planck_smica_cmb | planck_gnilc_dust | wmap_ilc9 | hi4pi_nhi (Default: alle im Manifest)")
    ap.add_argument("--trend", type=str, default="log", choices=("auto","log","inv"),
                    help="Trendmodell für kappa(s): auto wählt AIC-min(log, inv).")
    ap.add_argument("--scales", type=str, default=None,
                    help="Kommagetrennte Skalenliste, z.B. 1,2,4,8,16,32,64 (Default: Skript-Defaults).")
    ap.add_argument("--null", type=int, default=0,
                    help="Anzahl Phasen-Surrogates pro Patch (0 = aus).")
    ap.add_argument("--null-seed", type=int, default=20251,
                    help="RNG-Seed für Surrogates.")
    ap.add_argument(
        "--compressor",
        type=str,
        default="gzip",
    ...
    if args.dataset:
        ...
        # alle Datensätze, die im Manifest Patches haben; falls Manifest leer, nutze vorhandene Ordner
        if mani:
