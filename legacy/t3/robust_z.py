# scripts/t3/robust_z.py
# -*- coding: utf-8 -*-
"""
Robuste Z-Standardisierung pro Patch und Skala gegen die Nullfamilie.

Input:
- Real per-patch CSV:    <prefix>_metrics_per_patch.csv
- Null per-patch CSV:    <prefix>_null[_family]_metrics_per_patch.csv
  mit Spalten mindestens: rep, patch, s, bytes_per_cell
...
Wenn mad == 0:
- scale wird auf EPS gesetzt und mad0_flag = 1.
"""
...
def main() -> None:
    ...
    ap.add_argument("--null-per-patch", type=str, required=True, help="Pfad zu *_null*_metrics_per_patch.csv")
    args = ap.parse_args()
