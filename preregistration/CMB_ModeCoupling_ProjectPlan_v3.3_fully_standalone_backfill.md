# File: preregistration/CMB_ModeCoupling_ProjectPlan_v3.3_fully_standalone_backfill.md
# Project Plan v3.3 (Final, Pre-Registration Sealed, Fully Standalone, Backfill-Enabled)
...
This document supersedes v3.2 by adding a backfill policy for Section 5 and by making cosmetic clarifications only. No reported quantity, effect definition, or threshold is changed.
...
Backfill policy (binding for v3.3):

Backfill is permitted for missing or defective technical prerequisites that prevent execution of any section of this plan.
This includes missing external files, missing manifests, missing SHA256 ledger files, missing environment lock files, missing scripts, and bug-fix releases that restore the protocol as written.

Backfill is not permitted to change any scientific degree of freedom.
In particular, no effect definition, threshold, decision rule, seed, RNG rule, index convention, ordering rule, input selection rule, or any element of Sections 2–4 and 6–8 may be altered under backfill.

Backfill procedure (binding):

1. A dedicated addendum commit must introduce exactly one new file under:
   preregistration/addenda/

2. The file name must follow:
   ADDENDUM_YYYY-MM-DD_<topic>.md

3. The addendum commit may only add:
   (a) the addendum file,
   (b) missing secondary inputs required for execution,
   (c) their SHA256 ledger files.
   It must not modify any file referenced by Sections 2–4 and 6–8.

4. The addendum must document:
   (a) what was missing or defective,
   (b) why execution was blocked,
   (c) the exact fix applied and why it restores the frozen protocol,
   (d) the exact inputs used (paths and SHA256),
   (e) the output directory used for the backfilled execution.

If a section is not executed due to missing prerequisites, it must be marked as "pending backfill" in runinfo and in the manuscript narrative.
Primary results and decision gates remain fully frozen.
Section 5 is interpretative only and may be deferred under this policy without affecting primary validity.
...
No modification of any element in Sections 2–4 and 6–8 is allowed.
Section 5 is interpretative only and may be executed later under the backfill policy in Section 5.0 without changing any decision gate.
...
The value smoke_sha256 is stored as a text artifact at a hard-fixed repo-root path:

- gzip_smoketest_hash.txt (repo root)

Repo-root path rule (binding):

- The file gzip_smoketest_hash.txt must exist at the repository root (same directory as .git).
- The file must contain exactly one line: the lowercase SHA256 hex digest, with a trailing newline.
...
## 5.0 Backfill rule for 𝒩_FFP (binding)

Section 5 is interpretative only and does not alter decision gates.

Backfill is permitted.

Backfill commit rule (binding):

Before any FFP execution, a commit must exist that introduces the FFP manifest file and records its SHA256 in a ledger file.
This commit must not modify any primary section.

After the corresponding manifest ledger is committed, that manifest version is frozen and must not be modified.
Any extension of the FFP list must use a new manifest version and a new ledger file in a dedicated addendum commit.

FFP local data root (hard-fixed for this study):

- data/external/ffp10/

Manifest path safety rule (binding):

- Every manifest entry's local path must be located under the hard-fixed root data/external/ffp10/.
- The local path must not contain ".." path segments and must not be an absolute path.
- If any entry violates this rule, the Section 5 run is invalid and its outputs are not admissible.

Manifest path (hard-fixed):

- preregistration/external_inputs/ffp10_manifest_v1.json

Manifest SHA256 ledger (hard-fixed, single-line hex only):

- preregistration/external_inputs/sha256_ffp10_manifest_v1.txt

Ledger rule (binding):

- sha256_ffp10_manifest_v1.txt contains exactly the SHA256 hex digest of the manifest file contents (no filename, no extra columns).
- The pair (ffp10_manifest_v1.json, sha256_ffp10_manifest_v1.txt) defines a frozen manifest snapshot.

Manifest extension rule (binding):

- Extending the FFP list is permitted only by introducing a new manifest version file and a new ledger file in a dedicated addendum commit.
- Example naming convention:
  - preregistration/external_inputs/ffp10_manifest_v2.json
  - preregistration/external_inputs/sha256_ffp10_manifest_v2.txt
- Any Section 5 rerun that uses an extended manifest must:
  - use a new Section 5 output directory,
  - archive and keep previous Section 5 artifacts unchanged,
  - cite which manifest version was used in runinfo via the snapshot pair stored in the output directory.
- The original v1 snapshot remains frozen and must not be modified after its ledger is committed.

Manifest snapshot rule (binding for every Section 5 run):

- Before running Section 5, write a byte-identical copy of the manifest to the Section 5 output directory as ffp_manifest_used.json.
- Write the SHA256 hex digest of ffp_manifest_used.json to ffp_manifest_ledger_sha256.txt in the same output directory.
- Section 5 results must cite the pair (ffp_manifest_used.json, ffp_manifest_ledger_sha256.txt) in runinfo.

FFP manifest rule (binding):

The FFP manifest is permitted to be created after data acquisition.
It is treated as a frozen external input once committed.
...
- Use exactly the first N_FFP entries in manifest order.
- N_FFP = 100.
- If fewer than 100 entries exist at execution time, Section 5 may be executed as a partial engineering run.
- Such a run must be labeled in runinfo and in the manuscript as "FFP partial, pending backfill".
- No interpretative conclusion may be drawn unless N_FFP = 100 entries are present in the manifest snapshot used.
- If the manifest is later extended, Section 5 must be rerun in a new output directory.
- Earlier Section 5 artifacts remain archived and are not overwritten.
...
If the environment lock file is temporarily unavailable at first execution time, engineering runs may proceed.
Manuscript reporting remains blocked until the lock file and its SHA256 ledger are committed via addendum and the primary sections are re-executed under the locked environment.
...
- null_summary.json
- runinfo.json
- gzip_smoketest_hash.txt (repo root; committed reference input)
...
- git_commit
- backfill_status (one of: "none", "pending", "executed")
- backfill_addendum (path to preregistration/addenda/ADDENDUM_YYYY-MM-DD_<topic>.md, or empty if none)
- python_version
- numpy_version
- scipy_version
- healpy_version
